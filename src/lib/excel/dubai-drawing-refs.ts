...
export function applyDrawingRefsToWorkbook(wb, resolve = resolveDrawingRef) {
  ...
  wb.eachSheet(ws => {
    ...
    for (let r = 1; r <= ws.rowCount; r++) {
      ...
      const desc = String(ws.getRow(r).getCell(col.desc).value || '');
      const currentRef = String(ws.getRow(r).getCell(col.ref).value || '');
      const resolved = resolve({ item, desc, currentRef });
      if (typeof resolved === 'string' && resolved.length > 0) {
        ws.getRow(r).getCell(col.ref).value = resolved;
        populated++;
      } else {
        skipped++;
      }
