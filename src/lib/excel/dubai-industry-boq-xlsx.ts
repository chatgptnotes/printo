...
import { applyRatesToWorkbook } from './dubai-2026-rates';
import { applyAvlToWorkbook }   from './dubai-avl-brands';
import { applyDrawingRefsToWorkbook } from './dubai-drawing-refs';
import { floorForCable, deriveTypicalFloors } from '../electrical/derive-cable-paths';
import { canonFloorKey } from '../electrical/canonicalize';
import { applyProvenanceToWorkbook, provenanceMatrix, MEASURED_GLYPH, ALLOWANCE_GLYPH } from './dubai-data-provenance';
...
export async function generateDubaiIndustryBoqXlsx({ project, electrical, overrides = {}, options = {} }) {
  ...
  // Lighting is "measured" (drawing-derived) only when the AI extracted fixtures
  // from the legend; otherwise Bill 8 falls back to a template estimate. This
  // flag keeps the provenance glyphs + Cover source-summary honest.
  opts.measuredLighting = Array.isArray(electrical?.lighting_fixtures) && electrical.lighting_fixtures.length > 0;
  ...
  // One "Floor - <label>" tab per floor, after the Summary, re-cutting the
  // Bills' items by floor. These are a MEMORANDUM only — they are never pushed
  // into `totals`, so the Summary grand total is unaffected, and the rate pass
  // skips them (applyRatesToWorkbook only touches "Bill …" sheets).
  ...
  if (opts.applyProvenance !== false) {
    const pStats = applyProvenanceToWorkbook(wb, { measuredLighting: opts.measuredLighting });
  ...
  //    soft amber fill on F+G makes "needs pricing" rows jump out at the reviewer.
  flagUnpricedRows(wb);
...
function flagUnpricedRows(wb) {
  ...
  wb.eachSheet((ws) => {
    ...
    for (let r = 1; r <= ws.rowCount; r++) {
      ...
      ws.getRow(r).getCell(6).fill = fill;
      ws.getRow(r).getCell(7).fill = fill;
    }
  });
}
...
function billTotalRow(ws, r, billNo, lastRow) {
  ...
  ws.getCell(`G${r}`).style = { ...STYLE.billTotal, alignment: { horizontal: 'center', vertical: 'middle' } };
  ws.getCell(`G${r}`).numFmt = AED_FMT;
  ws.getCell(`H${r}`).value = '';
...
function buildCover(ws, meta, opts) {
  ...
  // stay consistent if either input is edited.
  if (meta.building.tcl_kw != null) {
    ...
    ws.getCell(`B${r}`).style = { ...STYLE.body, font: { ...FONT_BASE, bold: true } };
    ws.mergeCells(`C${r}:H${r}`);
    const mdResult = Math.round((meta.building.tcl_kw) * (meta.building.demand_factor ?? 0.75));
    ws.getCell(`C${r}`).value = { formula: `ROUND(C${tclRow}*C${dfRow},0)`, result: mdResult };
  ...
  r++;
  for (const [bill, label, glyph, note] of provenanceMatrix(opts)) {
...
function buildBill2(ws, meta, e, opts) {
  ...
  // 1000 kVA + 1500 kVA). Fall back to a single MD-sized line when none extracted.
  // Dedupe by (kVA, voltage ratio) — the SLD often labels two units "Transformer #1"
  // so an extractor can emit the same rating twice; collapse and sum counts.
  const txMap = new Map();
...
function buildBill3(ws, meta, e, opts) {
  ...
  const typical: any[] = [], ground: any[] = [], roof: any[] = [], ev: any[] = [], retail: any[] = [], emerg: any[] = [];
...
// ─── Bill 5 — LV Cables ───────────────────────────────────────────────────
// Bill 5 carries its own 9-column layout (adds a dedicated "Cable Size" column)
// so each cable line reads as a schedule — size in its own column, route in the
// description — instead of burying the size inside a repeated spec paragraph.
// Layout: Item | Cable Size | Description | Reference | Unit | Qty | Rate | Amount | Origin
function setBill5Header(ws, meta) {
  ws.columns = [
    { width: 7 },  // 1 Item
    { width: 28 }, // 2 Cable Size
    { width: 34 }, // 3 Description (route & scope)
    { width: 16 }, // 4 Reference
    { width: 7 },  // 5 Unit
    { width: 9 },  // 6 Qty
    { width: 12 }, // 7 Rate
    { width: 15 }, // 8 Amount
    { width: 22 }, // 9 Origin / Brand
  ];
  ws.views = [{ state: 'frozen', xSplit: 0, ySplit: 3, topLeftCell: 'A4', activeCell: 'A4' }];
  ws.pageSetup = {
  ...
  ws.headerFooter = {
    oddHeader: `&L&"Arial,Bold"&12${meta.project_name}&R&"Arial,Italic"&10Job ${meta.job_no || ''}`,
    oddFooter: `&LBill 5 — LV Power Cables&RPage &P of &N`,
  };
  ws.mergeCells('A1:I1');
  ws.getCell('A1').value = 'BILL No. 5 — LV POWER CABLES (Main + Distribution + Final Circuits)';
  ws.getCell('A1').style = STYLE.titleBar;
  ws.getRow(1).height = 27.75;
  ws.mergeCells('A2:I2');
  ws.getCell('A2').value = `Project: ${meta.project_name}, Plot ${meta.plot_no || '—'}, ${meta.location}  |  Job: ${meta.job_no || '—'}  |  Authority: ${meta.authority}`;
  ws.getCell('A2').style = { ...STYLE.body, font: { ...FONT_BASE, italic: true, size: 9 }, alignment: { vertical: 'middle', horizontal: 'left' } };
  ws.getRow(2).height = 13.5;
  const cols = ['Item', 'Cable Size', 'Description (Route & Scope)', 'Reference', 'Unit', 'Qty', 'Rate\n(AED)', 'Amount\n(AED)', 'Origin / Brand'];
  cols.forEach((c, i) => {
    const cell = ws.getRow(3).getCell(i + 1);
    cell.value = c;
    cell.style = STYLE.tableHead;
...
function styleFor5(col, alt) {
  if (col === 1)            return alt ? STYLE.bodyLeftBoldAlt : STYLE.bodyLeftBold;
  if (col === 5)            return alt ? STYLE.bodyCenterAlt   : STYLE.bodyCenter;
  if (col >= 6 && col <= 8) return alt ? STYLE.bodyRightAlt    : STYLE.bodyRight;
  return alt ? STYLE.bodyAlt : STYLE.body; // 2,3,4,9 left/wrap
}

function band5(ws, r, text) {
...
// One spec note printed ONCE under a band — the BS/IEC + installation boilerplate
// that used to repeat on every cable row.
function note5(ws, r, text) {
  ws.mergeCells(`A${r}:I${r}`);
  ws.getCell(`A${r}`).value = text;
  ws.getCell(`A${r}`).style = { ...STYLE.body, font: { ...FONT_BASE, italic: true, size: 9, color: { argb: 'FF555555' } }, alignment: { vertical: 'middle', wrapText: true, horizontal: 'left' } };
  ws.getRow(r).height = Math.min(60, Math.max(18, estimateWrappedLines(text, 150) * 12 + 6));
}

function cableRow5(ws, r, item, size, desc, ref, qty, unit = 'm', alt = false) {
  ...
  row.getCell(7).value = null; // Rate — unpriced (indicative BoQ)
  // Amount = Qty × Rate (cols F, G). Blank until both filled.
  row.getCell(8).value = { formula: `IF(OR(F${r}="",G${r}=""),"",F${r}*G${r})`, result: null };
  row.getCell(9).value = '';
  for (let c = 1; c <= 9; c++) row.getCell(c).style = styleFor5(c, alt);
  row.getCell(6).numFmt = '#,##0';
  row.getCell(7).numFmt = AED_FMT;
  row.getCell(8).numFmt = AED_FMT;
  const lines = Math.max(estimateWrappedLines(size, 26), estimateWrappedLines(desc, 40), estimateWrappedLines(ref, 18));
  row.height = Math.min(96, Math.max(18, lines * 14 + 4));
}

function subTotal5(ws, r, label, firstRow, lastRow) {
  ws.mergeCells(`A${r}:G${r}`);
  ws.getCell(`A${r}`).value = label;
  ws.getCell(`A${r}`).style = STYLE.subTotal;
  ws.getCell(`H${r}`).value = { formula: `SUM(H${firstRow}:H${lastRow})`, result: null };
  ws.getCell(`H${r}`).style = { ...STYLE.subTotal, alignment: { horizontal: 'center', vertical: 'middle' } };
  ws.getCell(`H${r}`).numFmt = AED_FMT;
  ws.getCell(`I${r}`).value = '';
  ws.getCell(`I${r}`).style = STYLE.subTotal;
  ws.getRow(r).height = 19.5;
}

function billTotal5(ws, r, lastRow) {
  r++; // gap row
  ws.mergeCells(`A${r}:G${r}`);
  ws.getCell(`A${r}`).value = 'BILL 5 — TOTAL CARRIED TO SUMMARY';
  ws.getCell(`A${r}`).style = STYLE.billTotal;
  ws.getCell(`H${r}`).value = { formula: `SUMIF(H4:H${lastRow},"<>")`, result: null };
  ws.getCell(`H${r}`).style = { ...STYLE.billTotal, alignment: { horizontal: 'center', vertical: 'middle' } };
  ws.getCell(`H${r}`).numFmt = AED_FMT;
  ws.getCell(`I${r}`).value = '';
  ws.getCell(`I${r}`).style = STYLE.billTotal;
  ws.getRow(r).height = 24;
  return r;
}

// Compact size label for the Cable Size column, e.g. "4C×150mm² XLPE/SWA/PVC + 1C×70mm² ECC".
function cableSizeLabel(c) {
  const cores = c.cores || 4;
  const t = (c.type || '').toLowerCase();
  const cd = String(c.circuit_description || '');
  // Single-core wires drawn in conduit ("4X1C 16mm² CU/PVC/WIRES") are NOT armoured
  // XLPE/SWA — they're a different product/rate, so label them as such. Detect by the
  // explicit annotation only ("WIRES" / "CU/PVC"); never off the "+1X1C … ECC" earth
  // token, which would wrongly flag ordinary multicore cables as singles.
  const isSingles = /\bwires?\b/i.test(cd) || /cu\s*\/\s*pvc/i.test(cd);
  const insul = /fire|fr|lszh/.test(t) ? 'FR/LSZH' : (isSingles ? 'CU/PVC (single-core, in conduit)' : 'XLPE/SWA/PVC');
  const coreLabel = isSingles ? `${cores}×1C` : `${cores}C`;
  let label = `${coreLabel}×${c.size_mm2}mm² ${insul}`;
  // Pull the "+ … ECC" token out of circuit_description if present — keep just the ECC,
  // not the whole repeated spec sentence.
  const ecc = String(c.circuit_description || '').match(/\d+\s*[Cx×]?\s*\d+(?:\.\d+)?\s*mm²?\s*ECC/i);
  if (ecc) label += ` + ${ecc[0].replace(/\s+/g, '').replace(/mm2/i, 'mm²')}`;
  return label;
}

// Best-effort size token for freeform cable text (bands 5.5 / 5.6).
function sizeToken(text) {
  const m = String(text || '').match(/\d+\s*C\s*[×x]?\s*\d+(?:\.\d+)?\s*mm²?/i);
  return m ? m[0].replace(/\s+/g, '').replace(/mm2/i, 'mm²') : '—';
}

const NOTE_XLPE = 'Spec (all rows in this section): XLPE / SWA / PVC 600/1000 V armoured cable to BS 5467 / IEC 60502, on plant / utility risers (non-public-area, non-escape-route). Installed in cable tray / ladder / conduit incl. fire stopping at penetrations, BW-type brass glands with shrouds, compression lugs, ferrules and terminations at both ends.';
const NOTE_FR = 'Spec (all rows in this section): Fire-Rated LSZH cable to BS 6387 CWZ / IEC 60331-21 (950 °C / 3 h circuit integrity); LSZH outer jacket per BS 7211 / IEC 60332-3 Cat C / IEC 61034 / IEC 60754. Dedicated fire-rated containment, FR cleats at max 300 mm centres, FR/LSZH glands, FR terminations, red ID labels.';

function buildBill5(ws, meta, e, opts) {
  setBill5Header(ws, meta);
  let r = 4;
  ...
  if (xlpe.length) {
    band5(ws, r++, '5.1 — XLPE / SWA / PVC LV CABLES — Plant & Utility Risers (LV Panel → SMDB)');
    note5(ws, r++, NOTE_XLPE);
    const f = r;
    xlpe.forEach((c, i) => {
      cableRow5(ws, r++, `5.1.${i + 1}`, cableSizeLabel(c),
        `${c.from} → ${c.to}${(c.floor ?? floorForCable(c.from, c.to)) ? ` (${c.floor ?? floorForCable(c.from, c.to)})` : ''} — plant / utility riser`,
        c.source_drawing_number || 'P-200', Math.round(c.length_m || 0), 'm', i % 2 === 1);
    });
    subTotal5(ws, r++, 'Sub-Total 5.1', f, r - 1); r++;
  }

  if (fr.length) {
    band5(ws, r++, '5.2 — FIRE-RATED LSZH CABLES (BS 6387 CWZ / IEC 60331-21 — life-safety circuits)');
    note5(ws, r++, NOTE_FR);
    const f = r;
    fr.forEach((c, i) => {
      cableRow5(ws, r++, `5.2.${i + 1}`, cableSizeLabel(c),
        `${c.from} → ${c.to}${(c.floor ?? floorForCable(c.from, c.to)) ? ` (${c.floor ?? floorForCable(c.from, c.to)})` : ''} — life-safety circuit`,
        c.source_drawing_number || 'P-200', Math.round(c.length_m || 0), 'm', i % 2 === 1);
    });
    subTotal5(ws, r++, 'Sub-Total 5.2', f, r - 1); r++;
  }
  ...
  if (distXLPE.length) {
    band5(ws, r++, '5.3 — DISTRIBUTION CABLES (SMDB → DB) — XLPE / SWA / PVC');
    note5(ws, r++, NOTE_XLPE);
    const f = r;
    distXLPE.forEach((c, i) => {
      cableRow5(ws, r++, `5.3.${i + 1}`, cableSizeLabel(c),
        `${c.from} → ${c.to}${(c.floor ?? floorForCable(c.from, c.to)) ? ` (${c.floor ?? floorForCable(c.from, c.to)})` : ''} — SMDB → DB distribution`,
        c.source_drawing_number || 'P-201', Math.round(c.length_m || 0), 'm', i % 2 === 1);
    });
    subTotal5(ws, r++, 'Sub-Total 5.3', f, r - 1); r++;
  }
  if (distFR.length) {
    band5(ws, r++, '5.4 — DISTRIBUTION CABLES (SMDB → DB) — Fire-Rated LSZH');
    note5(ws, r++, NOTE_FR);
    const f = r;
    distFR.forEach((c, i) => {
      cableRow5(ws, r++, `5.4.${i + 1}`, cableSizeLabel(c),
        `${c.from} → ${c.to}${(c.floor ?? floorForCable(c.from, c.to)) ? ` (${c.floor ?? floorForCable(c.from, c.to)})` : ''} — SMDB → DB (life-safety)`,
        c.source_drawing_number || 'P-201', Math.round(c.length_m || 0), 'm', i % 2 === 1);
    });
    subTotal5(ws, r++, 'Sub-Total 5.4', f, r - 1); r++;
  }

  // 5.5 — LSZH cables for public-area / escape-route final circuits (DCD requirement)
  band5(ws, r++, '5.5 — LSZH CABLES — Public-Area & Escape-Route Final Circuits (DCD compliance)');
  let f55 = r;
  const lszh_lines = [
    ['LSZH 4C × 16 mm²', 'Corridor & lobby final-circuit risers, escape-route mains (BS 7211 / IEC 60332-3 Cat C / IEC 61034 / IEC 60754).', 220],
    ['LSZH 3C × 4 mm² Cu', 'Apartment lighting & socket sub-mains in escape corridors.', 480],
    ['LSZH 3C × 2.5 mm² Cu', 'Final-circuit drops to apartment outlets & accessories on escape route.', 1200],
    ['LSZH 2C × 1.5 mm² Cu', 'Emergency / exit-sign final circuits (combined with FR per Bill 5.2).', 540],
  ];
  lszh_lines.forEach((d, i) => cableRow5(ws, r++, `5.5.${i + 1}`, d[0], d[1], 'BS 7211', d[2], 'm', i % 2 === 1));
  subTotal5(ws, r++, 'Sub-Total 5.5', f55, r - 1); r++;

  // 5.6 — Final-circuit bulk wiring (PVC, plant areas)
  if (e?.bulk_cables?.length) {
    band5(ws, r++, '5.6 — FINAL-CIRCUIT WIRING — Plant Areas (PVC, BS 6004)');
    const f = r;
    e.bulk_cables.forEach((b, i) => {
      cableRow5(ws, r++, `5.6.${i + 1}`, sizeToken(b.specification),
        b.application || b.specification, 'BS 6004', Math.round(b.estimated_length_m || 0), 'm', i % 2 === 1);
    });
    subTotal5(ws, r++, 'Sub-Total 5.6', f, r - 1); r++;
  }

  // 5.7 — ECC allowance alongside rising mains
  const totalMain = lvCables.reduce((s, c) => s + (Number(c.length_m) || 0), 0);
  if (totalMain > 0) {
    band5(ws, r++, '5.7 — EARTH CONTINUITY CONDUCTORS (ECC) ALONGSIDE RISING MAINS');
    const f = r;
    cableRow5(ws, r++, '5.7.1', '1C × 35 mm² G/Y XLPE ECC',
      'Alongside fire-rated rising mains (allowance).', 'BS 6004',
      Math.round(fr.reduce((s, c) => s + (Number(c.length_m) || 0), 0)), 'm', false);
    cableRow5(ws, r++, '5.7.2', '1C × 10–70 mm² G/Y XLPE ECC',
      'Alongside XLPE rising mains (allow per run).', 'BS 6004',
      Math.round(totalMain * 0.85), 'm', true);
    subTotal5(ws, r++, 'Sub-Total 5.7', f, r - 1); r++;
  }

  return billTotal5(ws, r, r - 1);
}
...
// Order floors basement → ground → typical (1F…) → roof for the take-off.
function floorRank(fl) {
  const s = String(fl || '').toUpperCase();
...
// Group take-off rows by their floor and return [floor, items[]] pairs ordered
// basement → ground → 1F… → roof (unrecognised floors last). Used by the
// per-floor bills (3 SMDBs, 4 DBs) the same way Bills 7 & 8 group outlets/lighting.
function groupByFloorSorted(items, getFloor) {
  const byFloor = new Map();
  for (const it of items) {
    const fl = ((getFloor(it) || 'Unassigned') + '').trim() || 'Unassigned';
    if (!byFloor.has(fl)) byFloor.set(fl, []);
    byFloor.get(fl).push(it);
  }
  return [...byFloor.keys()]
    .sort((a, b) => floorRank(a) - floorRank(b))
    .map((fl) => [fl, byFloor.get(fl)]);
}
...
function buildBill7(ws, meta, e, opts) {
  ...
  const outletsProvisional = !(e?.power_outlets?.length);
  const outlets = (e?.power_outlets?.length ? e.power_outlets : [
  ...
  // Floor-wise take-off when outlets carry a floor (this floor has X, that floor
  // has Y, … → bill total). Mirrors the per-floor lighting bill (Bill 8).
  if (outlets.some(o => o.floor)) {
    const byFloor = new Map();
    for (const o of outlets) {
      const fl = (o.floor || 'Unassigned').toString().trim() || 'Unassigned';
      if (!byFloor.has(fl)) byFloor.set(fl, []);
      byFloor.get(fl).push(o);
    }
    for (const floor of [...byFloor.keys()].sort((a, b) => floorRank(a) - floorRank(b))) {
      sec++;
      bandRow(ws, r++, `7.${sec} — ${String(floor).toUpperCase()} — SOCKET OUTLETS & SMALL POWER`);
      const ff = r;
      byFloor.get(floor).forEach((o, i) =>
        lineRow(ws, r++, `7.${sec}.${i + 1}`, o.description, 'BS 1363', o.unit || 'Nr', Math.round(o.estimated_qty || 0), '', i % 2 === 1));
      subTotalRow(ws, r++, `Sub-Total 7.${sec} — ${floor}`, ff, r - 1); r++;
    }
  } else {
    sec++;
    if (outletsProvisional) bandRow(ws, r++, '7.0 — GENERIC ESTIMATE (no outlets read from drawings — provisional allowance, verify before submission)');
    bandRow(ws, r++, `7.${sec} — SOCKET OUTLETS`);
    const f = r;
    const sockets = outlets.filter(o => /socket|outlet|service box|shaver/i.test(o.description));
    sockets.forEach((o, i) => lineRow(ws, r++, `7.${sec}.${i + 1}`, o.description, 'BS 1363', o.unit || 'Nr', o.estimated_qty || 1, outletsProvisional ? 'PROVISIONAL — generic estimate' : '', i % 2 === 1));
    subTotalRow(ws, r++, `Sub-Total 7.${sec}`, f, r - 1); r++;
  }

  sec++;
  bandRow(ws, r++, `7.${sec} — SWITCHES & CONTROLS`);
  let f = r;
  const switches = [
  ...
  switches.forEach((d, i) => lineRow(ws, r++, `7.${sec}.${i + 1}`, d[0], 'BS 3676', d[1], d[2], '', i % 2 === 1));
  subTotalRow(ws, r++, `Sub-Total 7.${sec}`, f, r - 1); r++;

  sec++;
  bandRow(ws, r++, `7.${sec} — ISOLATORS & EV CHARGER OUTLETS`);
  f = r;
  const iso = [
  ...
  iso.forEach((d, i) => lineRow(ws, r++, `7.${sec}.${i + 1}`, d[0], 'IEC 62196 / IEC 60947', d[1], d[2], '', i % 2 === 1));
  subTotalRow(ws, r++, `Sub-Total 7.${sec}`, f, r - 1); r++;
...
function buildBill8(ws, meta, e, opts) {
  ...
  // Preferred: fixtures read from THIS drawing's legend, counted per floor.
  const fixtures = Array.isArray(e?.lighting_fixtures) ? e.lighting_fixtures : [];
  if (fixtures.length) {
    const byFloor = new Map();
    for (const fx of fixtures) {
      const floor = ((fx.floor || 'Unassigned').toString().trim()) || 'Unassigned';
      if (!byFloor.has(floor)) byFloor.set(floor, []);
      byFloor.get(floor).push(fx);
    }
    let sec = 0;
    for (const [floor, list] of byFloor) {
      sec++;
      bandRow(ws, r++, `8.${sec} — ${String(floor).toUpperCase()} — LIGHTING FIXTURES`);
      const ff = r;
      list.forEach((fx, i) => {
        const desc = `${fx.description}${fx.provisional ? ' (provisional — final design per ID / client)' : ''}`;
        lineRow(ws, r++, `8.${sec}.${i + 1}`, desc, fx.type_ref || '—', 'Nr',
          Math.round(fx.qty || 0), fx.provisional ? 'Provisional' : 'IEC 60598', i % 2 === 1);
      });
      subTotalRow(ws, r++, `Sub-Total 8.${sec} — ${floor}`, ff, r - 1); r++;
    }
    return billTotalRow(ws, r, '8', r - 1);
  }

  // Fallback only: no lighting legend was extractable from the drawings — generic
  // area-based estimate. Banner makes the assumption explicit on the sheet.
  bandRow(ws, r++, '8.0 — GENERIC ESTIMATE (no fixture legend read from drawings — review against ELV/lighting layout)');
  ...
  interior.forEach((d, i) => lineRow(ws, r++, `8.1.${i + 1}`, d[0], 'IEC 60598', d[1], d[2], 'PROVISIONAL — generic estimate', i % 2 === 1));
  ...
  exterior.forEach((d, i) => lineRow(ws, r++, `8.2.${i + 1}`, d[0], 'IEC 60598', d[1], d[2], 'PROVISIONAL — generic estimate', i % 2 === 1));
  ...
  ctrls.forEach((d, i) => lineRow(ws, r++, `8.3.${i + 1}`, d[0], 'DEWA', d[1], d[2], 'PROVISIONAL — generic estimate', i % 2 === 1));
...
function buildBill9(ws, meta, e, opts) {
  ...
  const earthProvisional = !items.length;

  if (earthProvisional) bandRow(ws, r++, '9.0 — GENERIC ESTIMATE (no earthing read from drawings — provisional allowance, verify before submission)');
  ...
  main_earth.forEach((it, i) => lineRow(ws, r++, `9.1.${i + 1}`, it.description, 'BS 7430 / DEWA', it.unit || 'Nr', it.qty || 1, earthProvisional ? 'PROVISIONAL — generic estimate' : '', i % 2 === 1));
...
// Sheet name: "Floor - <label>", stripped of Excel-illegal chars (\ / ? * [ ] :)
// and capped at the 31-char worksheet-name limit.
...
function buildFloorTakeoff(ws, meta, e, opts, floorLabel) {
  ...
  // Re-brand the title bar as a memorandum so no-one mistakes it for a priced Bill.
  ...
  // Render one category band: header → line rows → quantity sub-total.
  // makeRow(item, i) => [desc, ref, unit, qty, remark]
  const renderBlock = (label, items, makeRow) => {
    ...
    items.forEach((it, i) => {
      ...
      // Rate (F) is left blank by lineRow, so the Amount (G) formula renders "".
  ...
  // 2 — Distribution Boards on this floor
  ...
  // 3 — Cables whose route resolves to this floor (length in metres)
  ...
  // 4 — Power outlets & accessories on this floor
...
function buildSummary(ws, meta, totals, opts) {
  ...
  bills.forEach(([k, d, s, n], i) => {
    ...
    // Bill 5 uses a 9-column layout, so its total sits in column H, not G.
    const totalCol = k === 5 ? 'H' : 'G';
    row.getCell(4).value = { formula: `'${sheetNameFor(k)}'!${totalCol}${totals[k]}`, result: 0 };
