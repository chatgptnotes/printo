 ...
 * @returns {number | null}
 */
export function lookupRate(row, overrides: { heavy?: number; submain?: number; final?: number } | null = null) {
...
export function applyRatesToWorkbook(wb, lookup = lookupRate) {
  ...
  wb.eachSheet(ws => {
    ...
    for (let r = 1; r <= ws.rowCount; r++) {
      ...
      // Prepend the dedicated Cable Size cell (Bill 5) so cableRate() still sees
      // the size/cores even though the route text moved to the Description column.
      ...
      const qty  = ws.getRow(r).getCell(col.qty).value;
      const rate = lookup({ item, desc, unit, qty });
      if (typeof rate === 'number' && Number.isFinite(rate) && rate >= 0) {
        ws.getRow(r).getCell(col.rate).value = rate;
        // Force the cell to render the AED format.
        ws.getRow(r).getCell(col.rate).numFmt = '#,##0.00';
        populated++;
      } else {
        skipped++;
      }
